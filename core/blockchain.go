...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	return NewBlockChainWithMultiVMConfig(db, cacheConfig, chainConfig, engine, []vm.Config{vmConfig}, shouldPreserve, txLookupLimit)
}

func NewBlockChainWithMultiVMConfig(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig []vm.Config, shouldPreserve func(block *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if len(vmConfig) == 0 {
		vmConfig = []vm.Config{{}}
	}
	if cacheConfig == nil {
