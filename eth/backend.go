...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	var (
		...
		vmConfigHuobi = vm.Config{
			Debug:                   true,
			Tracer:                  logger.NewHuobiLogger(&logger.Config{EnableMemory: false, DisableStack: false, DisableStorage: true, Debug: true}),
			EnablePreimageRecording: config.EnablePreimageRecording,
		}
	)

	eth.blockchain, err = core.NewBlockChainWithMultiVMConfig(chainDb, cacheConfig, chainConfig, eth.engine, []vm.Config{vmConfig, vmConfigHuobi}, eth.shouldPreserve, &config.TxLookupLimit)
	if err != nil {
		return nil, err
	}
