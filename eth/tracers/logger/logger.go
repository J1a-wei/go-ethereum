...
func (l *StructLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	log := StructLog{contract.Address(), pc, op, gas, cost, mem, memory.Len(), stck, rdata, storage, depth, l.env.StateDB.GetRefund(), err}
...
type ExecutionResult struct {
	...
	StructLogs  []StructLogResLogger `json:"structLogs"`
}

// StructLogResLogger stores a structured log emitted by the EVM while replaying a
// transaction in debug mode
type StructLogResLogger struct {
	Pc            uint64             `json:"pc"`
	Op            string             `json:"op"`
	Gas           uint64             `json:"gas"`
	GasCost       uint64             `json:"gasCost"`
...
func formatLogs(logs []StructLog) []StructLogResLogger {
	formatted := make([]StructLogResLogger, len(logs))
	for index, trace := range logs {
		formatted[index] = StructLogResLogger{
			Pc:            trace.Pc,
			Op:            trace.Op.String(),
			Gas:           trace.Gas,
